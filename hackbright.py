...
def get_project_by_title(title):
    ...
    print("Description: {} \n Max_grade: {}".format(row[0], row[1]))

def get_grade_by_github_title(github, title):
    """Print grade student received for a project."""
    QUERY = """
                SELECT student_github, grade FROM grades
...
def assign_grade(github, title, grade):
    ...
    QUERY = """
        ...
        """
    db.session.execute(QUERY, {'grade' : grade, 'student_github' : github, 'project_title': title})

    db.session.commit() 

    print("{}'s {} project has been updated to {}".format(github, title, grade))
...
def add_project(title, description, max_grade):
    ...
    print("{}: {} with max grade of {} has been added!".format(title, description, max_grade))
